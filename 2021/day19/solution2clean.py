...
def parseInput():
	...
	for line in f:
		...
		if title:
			...
			cur_scanner.append(tuple([int(x) for x in line.split(',')]))
	scanners[sc_num] = cur_scanner # add the last one
	return scanners
...
def attempt(g1, g2):
	...
	for p1 in g1:
		for p2 in g2:
			g1_trans = set([subtract(g1p, p1) for g1p in g1])
			g2_trans = set([subtract(g2p, p2) for g2p in g2])

			overlap = g1_trans & g2_trans
			if len(overlap) >= 12:
...
def getpermute(pts):
	...
	pgroups = []
	for pt in pts:
		lps = [v for v in sequence(pt)]
		for i in range(len(lps)):
			out[i].append(lps[i])
	return out

persmap = {}
def do_stuff(scanners, i, j, persmap, permutes):
	best = []
	seq = None
	for k,v in permutes[j].items():
	...
	if len(best) < 12:
		...
		p1,p2 = next(iter(best))
		rez = subtract(p1,p2)
		persmap[(i, j)] = [rez, seq]
		print(f'{i}->{j}: best has {len(best)}, k is {seq}, map: {persmap[(i, j)]}')

# dict with v: 0-38 -> list(tuples(x,y,z))
scanners = parseInput()
# pprint.pp(scanners)
...
for i in range(len(scanners)):
	permutes[i] = getpermute(scanners[i])
...
for i in range(S):
	for j in range(S):
		if i != j:
			do_stuff(scanners,i,j,persmap, permutes)
...
def transToRefFrame(scanner_index):
	path = complete[scanner_index]
	# print(scanner_index, path)
	offset = offsets[scanner_index]
	...
	out = []
	for p in scanners[scanner_index]:
		trans = recurse(seqs, 0, p)
		out.append(add(offset, trans))
	
	return out

allpoints = set(scanners[0])
for s in range(1, S): allpoints.update(transToRefFrame(s))

print('part1', len(allpoints))
...
# would be nice to go in reverse :hmmmm:

# generate a map of permutation_sequences to their inverses
# that is, given permutation i on pt, which permutation j will undo it?
# pt = (1,2,3)
# permute2inverse = {}
# for i,ptp in enumerate(sequence(pt)):
# 	for j,ptpp in enumerate(sequence(ptp)):
# 		if pt == ptpp:
# 			permute2inverse[i] = j

# pprint.pp(permute2inverse)

# specials = []
# for i,j in permute2inverse.items():
# 	if i==j:
# 		specials.append(i)
# for s in specials:
# 	ptp = getseqforF(s, pt)
# 	ptpp = getseqforF(s, ptp)
# 	print(pt, ptp, ptpp)
...
# seq_inv = permute2inverse[oseq]
# print(opt, getseqforF(seq_inv, opt))
