...
for dir in dirs:
	...
	if turnon: cubes2.add(newcube)

	existingcubes = list(cubes)
	while len(existingcubes) > 0:
		existingcube = existingcubes.pop(0)
		...
		# for over in dims: print(over)

		noEConly = all(EConly==[] for (EConly,_, _) in dims)
		if noEConly:
			pass
